...
mosaic_t *source_mosaic;
mosaic_t *target_mosaic;
...
compare_moviepixel(int previousfile, int currentfile)
{
    ...
    {
      if (pixel->fileno == previousfile)
        previous=pixel;
      else if (pixel->fileno == currentfile)
        current=pixel;
      if (previous && current)
        break;
    }
    ...
    coeffs_t coeffs;
    memcpy(coeffs.subpixel.subpixels,previous->subpixels,NUM_SUBPIXELS * NUM_CHANNELS);
    return subpixel_compare (&coeffs, current, FLT_MAX);
}
...
void
generate_search_coeffs_for_subimage (coeffs_t *coeffs, unsigned char *image_data,
...
generate_global_classic (classic_reader_t *reader, int metric)
{
    ...
    for (ignore_forbidden = 0; ignore_forbidden < 2; ++ignore_forbidden)
    {
      for (i = 0; i < num_matches; ++i)
      {
          ...
          if (mosaic->matches[index].pixel == 0)
          {
            ...
            /* Avoid images in the vicinity */
            /*
            if (moviemode) {
              for (pixel = first_pixel; pixel != 0; pixel = pixel->next) {
                if ((matches[i].pixel->fileno - pixel->fileno < 3) &&
                    (matches[i].pixel->fileno - pixel->fileno > -3))
                  pixel->flag = 1;
              }
            }*/
            fprintf(stdout,"Match:%s\n",matches[i].pixel->filename);
    ...
    fprintf(stdout,"locations filled:%i\n",num_locations_filled);
...
read_tables (const char *library_dir)
{
  ...
  reset_pools(&pools);
    ...
    if (moviemode) {
      char filetmp[9];
      char *tmpfile = strstr(filename, "output-");
      strncpy(filetmp,tmpfile+7,9);
      int fileno = atoi(filetmp);
      pixel->fileno=fileno;
    }
    for (channel = 0; channel < NUM_CHANNELS; ++channel)
...
static int
initialize_output_name (char *input_name)
{
  char *filename;
  char filetmp[9];
  filename = strstr(input_name, "output-");
  strncpy(filetmp,filename+7,9);
  int fileno = atoi(filetmp);
  return fileno;
}

static void
get_target_filename(char **filename, int increment, const char *suffix)
{
  char filetmp[9];
  char newfile[1024];
  char *tmpfile;
  
  tmpfile = strstr(*filename, "output-");
  strncpy(filetmp,tmpfile+7,9);
  int fileno = atoi(filetmp);
  fileno = fileno+increment;
  if (fileno < 1)
    fileno=1;
  sprintf(newfile,"output-%09i%s",fileno,suffix);
  newfile[strlen(newfile) + 1] = '\0';
  strncpy(tmpfile,newfile,strlen(newfile));
}

static void
generate_image_sequence (mosaic_t *mosaic, char *input_name,
                         char *output_name, int cheat, int reverse_order,
                         int outfile)
{
  char *filename;
  char filetmp[9];
  ...
  for (y = 0; y < metaheight; ++y)
  {
    for (x = 0; x < metawidth; ++x)
    {
      filename = strstr(mosaic->matches[y * metawidth + x].pixel->filename,
                        "output-");
      strncpy(filetmp,filename+7,9);
      int fileno = atoi(filetmp);
      if (reverse_order) {
        ...
        fileno++;
      sprintf(newfile,"output-%09i.jpg.png",fileno);
      newfile[strlen(newfile) + 1] = '\0';
      strncpy(filename,newfile,strlen(newfile));

      if (scene_has_cut(fileno-1,fileno))
        mosaic->matches[y * metawidth + x].segued=1;
    }
  }

  sprintf(outfilename,"imagestmp/output-%09i.png",outfile);
  paste_classic(mosaic, input_name, outfilename, cheat);
}

#define SEGUE_PORTION 2
#define TARGET_PORTION
#define SOURCE_PORTION

void
segue_image_sequence(char *input_name, char *output_name, int cheat)
{
  int position_in_segue=-((no_of_frames/SEGUE_PORTION));
  int outfile, x, y, z, segued=0, total_segued=0;
  int mw = target_mosaic->metawidth,
      mh = target_mosaic->metaheight;
  char *filename;
  outfile = initialize_output_name(input_name);
  
  for (y = 0; y < source_mosaic->metaheight; ++y)
    for (x = 0; x < source_mosaic->metawidth; ++x)
      if (source_mosaic->matches[y * source_mosaic->metawidth + x].segued==1)
        total_segued++;

  int total_pixels_to_segue = (mh * mw)-total_segued;
  int pixels_to_segue_per_frame = (total_pixels_to_segue /
                                  ((no_of_frames/SEGUE_PORTION)));

  for (z = 0; z < ((no_of_frames/SEGUE_PORTION)+1); ++z)
  {
    segued=0;
    /* segue a single frame */
    while (segued < pixels_to_segue_per_frame) {
      int yrnd=(random() % 2);
      for ((yrnd)?(y = 0):(y = (mh-1));
           (yrnd)?(y < mh):(y > -1); (yrnd)?(++y):(--y))
      {
        int xrnd=(random() % 2);
        for ((xrnd)?(x = 0):(x = (mw-1));
             (xrnd)?(x < mw):(x > -1); (xrnd)?(++x):(--x))
        {
          if (segued >= pixels_to_segue_per_frame)
            goto done;
          if (target_mosaic->matches[y * mw + x].segued)
            continue;
          if (random() % 2)
            continue;
          match_t *m = target_mosaic->matches;
          filename = strdup(m[y * mw + x].pixel->filename);
          get_target_filename(&filename,position_in_segue,".jpg.png");
          source_mosaic->matches[y * mw + x].pixel->filename=filename;
          m[y * mw + x].segued = 1;
          segued++; total_segued++;
          (xrnd)?(x=mw):(x=-1); /*We've changed one frame in this row,
                                         so go to the next */
        }
      }
    }
    /*write out the segued frame*/
done:
    position_in_segue++;
    generate_image_sequence(source_mosaic, input_name, output_name, cheat,
                            0, outfile);
    outfile++;
  }
}
...
make_classic_mosaic (char *in_image_name, char *out_image_name,
         ...
         int first_in_sequence, int movie_sequence)
{
    mosaic_t *mosaic;
    int x, y;
  int i;
  ...
  fclose(protocol_out);
    ...
    if (movie_sequence) {
      int outfile;
      char outfilename[1024];
      outfile = initialize_output_name(in_image_name);
      sprintf(outfilename,"imagestmp/output-%09i.png",outfile);
      paste_classic(mosaic, in_image_name, outfilename, cheat);
      (first_in_sequence)?outfile++:outfile--;
      for (i = 0; i < (no_of_frames / 4); ++i)
      {
        generate_image_sequence(mosaic, in_image_name, out_image_name, cheat,
                                !first_in_sequence, outfile);
        (first_in_sequence)?outfile++:outfile--;
      }
...
static void reread_tables(string_list_t *library_directories) {
  string_list_t *lst;
  first_pixel = 0;
...
main (int argc, char *argv[])
{
  ...
  static struct option long_options[] =
    ...
    { "frames", required_argument, 0, 'z' },
    { "moviemode", no_argument, 0, 'n' },
  ...
  option = getopt_long(argc, argv, "l:m:e:z:w:h:y:i:n:q:s:cd:a:x:f",
         ...
         metric, cheat);
          ...
          if (!moviemode)
            make_classic_mosaic(argv[optind], argv[optind + 1],
                        ...
                        in_filename, out_filename, 0, 0);
          else {
            char *middle_frame=strdup(argv[optind]);
            char *last_frame=strdup(argv[optind]);
            /* Do the first 1/3 of the sequence */
            make_classic_mosaic(argv[optind], argv[optind + 1],
                        metric, scale, search, classic_min_distance, cheat,
                        in_filename, out_filename, 1, moviemode);
            /* Do the last 1/3 of the sequence */
            reread_tables(library_directories);
            get_target_filename(&last_frame, no_of_frames,".jpg");
            make_classic_mosaic(last_frame, argv[optind + 1],
                        metric, scale, search, classic_min_distance, cheat,
                        in_filename, out_filename, 0, moviemode);
            /* Do the middle 1/3 of the sequence */
            reread_tables(library_directories);
            get_target_filename(&middle_frame,(no_of_frames/4),".jpg");
            segue_image_sequence(middle_frame, argv[optind + 1], cheat);
          }
       }
  }
  else if (mode == MODE_BATCH)
  {
    ...
    if (type != LISP_TYPE_EOF && type != LISP_TYPE_PARSE_ERROR)
    {
      ...
      make_classic_mosaic(this_image_in_filename, this_image_out_filename,
              ...
              this_prot_in_filename, this_prot_out_filename, 0, 0);
        }
        else
        {
      fprintf(stderr, "unknown expression ");
