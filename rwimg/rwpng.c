...
open_png_file_reading (const char *filename, int *width, int *height)
{
    ...
    *height = png_get_image_height(data->png_ptr, data->info_ptr);
    ...
    assert(png_get_bit_depth(data->png_ptr, data->info_ptr) == 8 || png_get_bit_depth(data->png_ptr, data->info_ptr) == 16);
    assert(png_get_color_type(data->png_ptr, data->info_ptr) == PNG_COLOR_TYPE_RGB || png_get_color_type(data->png_ptr, data->info_ptr) == PNG_COLOR_TYPE_RGB_ALPHA);
    assert(png_get_interlace_type(data->png_ptr, data->info_ptr) == PNG_INTERLACE_NONE);
...
png_read_lines (void *_data, unsigned char *lines, int num_lines)
{
    ...
    for (i = 0; i < num_lines; ++i)
    {
        ...
        lines[i * png_get_image_width(data->png_ptr, data->info_ptr) * 3 + j * 3 + channel] = row[j * spp * bps + channel * bps];
    }
